...
--[[ Config Zombie Models ]] local Models = {
    "u_f_m_corpse_01",
    "u_f_y_corpse_01",
    "u_m_y_corpse_01",
    "u_m_y_zombie_01",
...
--[[ Config Zombie Walk Styles  ]] local walks = {
    ...
    function()
        ...
        while true do
            ...
            if #entitys < Config.SpawnZombie then
                ...
                while not HasModelLoaded(GetHashKey(EntityModel)) or
                    not HasCollisionForModelLoaded(GetHashKey(EntityModel)) do
                    Wait(1)
                end
                ...
                repeat
                    ...
                    for _, player in pairs(players) do
                        ...
                        if
                            posX > playerX - Config.MinSpawnDistance and posX < playerX + Config.MinSpawnDistance or
                                posY > playerY - Config.MinSpawnDistance and posY < playerY + Config.MinSpawnDistance
                         then
                            canSpawn = false
                            break
                        else
                ...
                until canSpawn
                 --
                --[[ Spawns Zombie ]] entity = CreatePed(4, GetHashKey(EntityModel), posX, posY, posZ, 0.0, true, false)
            ...
            for i, entity in pairs(entitys) do
                if not DoesEntityExist(entity) then
                    ...
                    if
                        pedX < playerX - Config.DespawnDistance or pedX > playerX + Config.DespawnDistance or
                            pedY < playerY - Config.DespawnDistance or
                            pedY > playerY + Config.DespawnDistance
                     then
...
--[[ Zombie sounds ]] Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(10000)
            for i, entity in pairs(entitys) do
...
--[[ No Health Recharge Function ]] if Config.NotHealthRecharge then
    SetPlayerHealthRechargeMultiplier(PlayerId(), 0.0)
end
 --

--[[ Mute Ambience Function ]] if Config.MuteAmbience then
    StartAudioScene("CHARACTER_CHANGE_IN_SKY_SCENE")
end
 --

--[[ Set Player Stats Function ]] Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(0)
...
--[[ Zombie Loot Drop Function ]] if Config.ZombieDropLoot then
    Citizen.CreateThread(
        function()
            while true do
                Citizen.Wait(1)
                                ...
                                if (Vdist(playerX, playerY, playerZ, pedX, pedY, pedZ) < 4.0) then
                                    ...
                                    if IsControlJustReleased(1, 51) then
                                        if DoesEntityExist(GetPlayerPed(-1)) then
                                            ...
                                            elseif
                                                randomChance >= Config.ProbabilityMoneyLoot and
                                                    randomChance < Config.ProbabilityItemLoot
                                             then
...
--[[ Set Safe Zone Areas Function  ]] if Config.SafeZone then
    Citizen.CreateThread(
        function()
            while true do
                Citizen.Wait(1)
...
function chaseAfterPerson()
	for i, entity in pairs(entitys) do		
		local currentVehicle = GetVehiclePedIsIn(PlayerPedId(), false)
        if currentVehicle ~= 0 then
			TaskGoToEntity(entity, currentVehicle, -1, 0.0, 4.0, 1073741824, 0)
			SetPedMoveRateOverride(entity, 10.0)
        else
            TaskGoToEntity(entity, GetPlayerPed(-1), -1, 0.0, 4.0, 1073741824, 0)
        end
	end
end

--[[ Safe Zone Blip Function ]] if Config.SafeZoneRadioBlip then
    Citizen.CreateThread(
        function()
            for k, v in pairs(Config.SafeZoneCoords) do
...
--[[ Clear All Zombies Function ]] RegisterNetEvent("qb-zombies:clear")
AddEventHandler(
    "qb-zombies:clear",
    function()
        for i, entity in pairs(entitys) do
...
--[[ Debug Function  ]] if Config.Debug then
    Citizen.CreateThread(
        function()
            while true do
                Citizen.Wait(1)
...
--[[ No PEDs Function ]] if Config.NoPeds then
    Citizen.CreateThread(
        function()
            while true do
                Citizen.Wait(1)
